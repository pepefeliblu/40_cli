...
bool Cortana::checkCaida(Carta40 &deMano, std::vector<Carta40> &mesa) {
  for (int i = 0; i < mesa.size(); i++) {
    // Si los valores de la carta en mano y mesa coinciden; y fue la última en
    // jugarse
...
bool Cortana::checkSuma(Carta40 &sumar, std::vector<Carta40> &mesa_) {
  ...
  for (int i = 0; i < mesa_.size() - 1; i++) {
    for (int j = i + 1; j < mesa_.size(); j++) {
      ...
      // Si la suma es menor a 7 (Valor que representa a la J) y coincide con la
      // carta lanzada hay suma
...
// Escoge una carta al azar basado en el tamaño de la mano
