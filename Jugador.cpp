...
#include <algorithm>
#include <iostream>
//El constructor asigna 0 puntos y el estado de ganar como falso, asigna el nombre dado
...
void Jugador::jugarCarta(Carta40& escogida, std::vector<Carta40>& mesa)
{
	...
	if(!mesa.empty())
	{
		//Se asegura de que todas las cartas no sean las ultimas en ser lanzadas
...
void Jugador::chupar(Carta40& deMano, std::vector<Carta40>& mesa)
{
	...
	for(int i =0; i < mesa.size(); i++)
	{
		//Si encuentra una de igual valor
	...
	if(!mesa.empty())
	{
		//Se asegura de que todas las cartas de la mesa no sean las ultimas en lanzarse
		for(int i =0; i < mesa.size(); i++)
		{
			mesa[i].setUltima(false);
		}
...
void Jugador::escalera(Carta40& deMano, std::vector<Carta40>& mesa)
{
	...
	//Si se cumple la condición pasa la ultima carta al cartón y al resto de la escalera
	if(existe)
	{
...
void Jugador::suma(Carta40& sumar, std::vector<Carta40>& mesa_)
{
	...
	for(int i = 0; i < mesa_.size()-1; i++)
	{
		for(int j= i+1; j < mesa_.size(); j++)
		{
			...
			//Revisa que la suma sea menor 7 (valor de J) y  coincida con el valor de la carta lanzada
...
//Parte el mazo 
void Jugador::partir(std::vector<Carta40>& mazo)
{	//iterador que guarda la posción del elemento en la mitad
	std::size_t mitad = mazo.size()/2;
	//se crea un areglo que guarda los elementos de la primera mitad
...
void Jugador::repartir(Jugador *sujeto, std::vector<Carta40>& mazo)
{
	...
	//se pasan las cartas al arrelgo mano del jugador
	sujeto->setMano(manoTemp);
}
//Copia un arreglo de cartas a la mano del jugador
void Jugador::setMano(std::vector<Carta40>& mano_)
{
	mano= mano_;
}
//Devuelve un elemento de la mano basado en el índice
Carta40 &Jugador::getMano(int val)
{
	return mano[val];
 } 
...
{	
	...
	std::cout << std::endl << "El jugador " << getNombre() << " tiene un puntaje de: " << getPuntaje() << std::endl;
}
	...
	for(int i = 0; i < 3; i++)
	{
		...
		for(int j = i+1; j-1 < mano.size(); j++)
		{
			...
			if(contador ==3)
			{
				...
				std::cout << std::endl << "2 por guapo para: " << getNombre() << std:: endl;
				break;
			}
		}
		contador = 1;
...
void Jugador::calcularCarton()
{
	...
	std::cout << std::endl << getNombre() << " tiene " << carton.size() << " cartas\n";
...
void Jugador::setGanar(bool status)
{
	ganar = status;
}

bool Jugador::getGanar()
{
	return ganar;
}

void Jugador::setNombre(std::string nombre_)
{
	nombre = nombre_;
}
...
{
	mesa_.erase(std::remove(mesa_.begin(), mesa_.end(), seleccionada), mesa_.end());
}
int Jugador::cantCarta()
{
